...
const CATEGORY_AZ = { medicine: 'Dərman', equipment: 'Avadanlıq', consumable: 'Sərfiyyat', office: 'Ofis', other: 'Digər' }
  ...
  return (
    <AdminLayout activePage="anbar">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#0f1b2d' }}>Anbar</h1>
          <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: 13 }}>{total} məhsul</p>
        </div>
      ...
      {lowStockItems.length > 0 && (
        <div
          ...
          ⚠️ {lowStockItems.length} məhsulun stoku azdır
          <span style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 500, opacity: 0.7 }}>{showLowStock ? '▲ Gizlət' : '▼ Göstər'}</span>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 14, marginBottom: 24 }}>
        {[
          { label: 'Ümumi məhsul',  value: total,                    accent: '#00848e' },
          { label: 'Az stok',       value: lowStockItems.length,      accent: lowStockItems.length > 0 ? '#dc2626' : '#16a34a' },
          { label: 'Dərman',        value: countCat('medicine'),      accent: '#2563eb' },
          { label: 'Avadanlıq',     value: countCat('equipment'),     accent: '#16a34a' },
          { label: 'Sərfiyyat',     value: countCat('consumable'),    accent: '#ca8a04' },
        ].map(c => (
      ...
      {/* ── Main table ─────────────────────────────────────── */}
      <ItemTable items={items} loading={loading} onStockIn={i => openStock(i,'in')} onStockOut={i => openStock(i,'out')} onEdit={openEdit} onDelete={handleDelete} />
      ...
      {showLowStock && lowStockItems.length > 0 && (
        <div style={{ marginTop: 28 }}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: '#c2410c', marginBottom: 12 }}>⚠️ Az Stok Məhsullar</h2>
          <ItemTable
            items={lowStockItems}
            loading={false}
      ...
      {showModal && (
        <Overlay onClose={() => setShowModal(false)}>
          <div style={{ background: 'white', borderRadius: 16, width: 540, maxWidth: '95vw', maxHeight: '90vh', overflow: 'auto', padding: 28 }}>
            <ModalHeader title={editItem ? 'Məhsulu Redaktə Et' : 'Yeni Məhsul'} onClose={() => setShowModal(false)} />
            {formErr && <ErrBox msg={formErr} />}

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div style={{ gridColumn: '1/-1' }}>
                <label style={lbl}>Məhsul adı *</label>
              ...
              <div>
                <label style={lbl}>Vahid</label>
                <input style={inp} value={form.unit} onChange={e => setF('unit', e.target.value)} placeholder="ədəd / ml / mg / litr / qab" />
              </div>
            ...
            <ModalFooter onClose={() => setShowModal(false)} onSave={handleSave} saving={saving} saveLabel={editItem ? 'Yadda saxla' : 'Əlavə et'} />
          </div>
        </Overlay>
      )}
      ...
      {showStockModal && stockItem && (
        <Overlay onClose={() => setShowStockModal(false)}>
          <div style={{ background: 'white', borderRadius: 16, width: 400, maxWidth: '95vw', padding: 28 }}>
            <ModalHeader
              title={stockType === 'in' ? 'Stok Girişi' : 'Stok Çıxışı'}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <input style={inp} placeholder="İstəyə bağlı qeyd..." value={stockNote} onChange={e => setStockNote(e.target.value)} />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 22, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleStock} disabled={stockSaving}
                ...
                {stockSaving ? 'Saxlanır...' : stockType === 'in' ? '+ Giriş' : '- Çıxış'}
              </button>
            </div>
          </div>
        </Overlay>
...
function ItemTable({ items, loading, onStockIn, onStockOut, onEdit, onDelete, headerStyle = {} }) {
  ...
  return (
    <div style={{ background: 'white', borderRadius: 14, border: '1px solid #f1f5f9', overflow: 'hidden' }}>
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 60, color: '#94a3b8', fontSize: 14 }}>Məhsul tapılmadı</div>
      ) : (
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f8fafc', ...headerStyle }}>
              {['Məhsul', 'Kateqoriya', 'Stok', 'Min Stok', 'Qiymət', 'Status', ''].map(h => (
                ...
                <tr key={item._id} style={{ transition: 'background .1s' }}
                  ...
                  <td style={{ ...tdStyle, whiteSpace: 'nowrap' }}>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      <ABtn label="+ Giriş"  color="#16a34a" onClick={() => onStockIn(item)} />
                      <ABtn label="- Çıxış"  color="#ea580c" onClick={() => onStockOut(item)} />
...
const ModalFooter = ({ onClose, onSave, saving, saveLabel = 'Saxla' }) => (
  <div style={{ display: 'flex', gap: 10, marginTop: 22, justifyContent: 'flex-end' }}>
