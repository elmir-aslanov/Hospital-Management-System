...
const TYPE_LABELS = {
  lab_result: 'Laboratoriya nəticəsi', imaging: 'Görüntüləmə',
  clinical_note: 'Klinik qeyd', procedure: 'Prosedur',
  vaccination: 'Peyvənd', other: 'Digər',
};
...
export default function ENeticePage() {
  ...
  async function search(params) {
    ...
    try {
      ...
      if (s === 404)      setError('Pasiyent tapılmadı. ID-nizi yoxlayın.');
      else if (s === 401) setError('Məlumatlar uyğun gəlmir. Doğum tarixinizi yoxlayın.');
      else if (s === 400) setError(err.response?.data?.message || 'Zəhmət olmasa məlumatları doldurun.');
      else                setError('Server xətası. Bir az sonra yenidən cəhd edin.');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f0fafa 0%, #e8f4f8 100%)', fontFamily: FONT, padding: '40px 16px' }}>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <div className="no-print" style={{ display: 'flex', gap: 10, marginBottom: 20, justifyContent: 'flex-end' }}>
            <button onClick={() => window.print()} style={{ padding: '10px 22px', background: TEAL, border: 'none', borderRadius: 10, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}>PDF Yüklə</button>
            <button onClick={reset} style={{ padding: '10px 22px', background: 'transparent', border: `2px solid ${TEAL}`, borderRadius: 10, color: TEAL, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}>Yeni Sorğu</button>
          </div>
          <div id="print-results">
            ...
            <div style={{ background: `linear-gradient(135deg, ${NAVY} 0%, ${TEAL} 100%)`, borderRadius: 16, padding: '24px 28px', marginBottom: 24, color: '#fff' }}>
              ...
              <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap' }}>
                {[['Pasiyent ID', patient.patientId || '—'], ['Doğum tarixi', fmt(patient.dateOfBirth)], ['Telefon', patient.phone || '—'], ['Nəticə sayı', records.length]].map(([k, v]) => (
                  <div key={k}>
            ...
            {records.length === 0 ? (
              <div style={{ background: '#fff', borderRadius: 14, padding: 40, textAlign: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.06)' }}>
                ...
                <p style={{ fontSize: 15, color: '#718096', margin: 0 }}>Nəticə tapılmadı. Zəhmət olmasa laboratoriya ilə əlaqə saxlayın.</p>
              </div>
            ) : (
                ...
                {records.map((rec, i) => (
                  <div key={rec._id || i} style={{ background: '#fff', borderRadius: 14, padding: '20px 24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderLeft: `4px solid ${TEAL}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      <div>
                        <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: '#e0f2fe', color: '#0369a1', display: 'inline-block' }}>{TYPE_LABELS[rec.type] || rec.type}</span>
                    ...
                    {rec.doctorId?.specialization && <div style={{ fontSize: 12, color: '#718096' }}>İxtisas: <strong>{rec.doctorId.specialization}</strong></div>}
                    {rec.attachments?.length > 0 && (
                      <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {rec.attachments.map((url, j) => (
                          <a key={j} href={url} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: TEAL, fontWeight: 600, textDecoration: 'none', border: `1px solid ${TEAL}`, borderRadius: 6, padding: '3px 10px' }}>Fayl {j + 1} ↗</a>
                        ))}
                      </div>
                    )}
                  </div>
      ...
      <div style={{ width: '100%', maxWidth: 480, borderRadius: 20, padding: '32px 36px', boxShadow: '0 8px 40px rgba(0,0,0,0.10)', background: 'white' }}>
        ...
        <form onSubmit={handleIdSearch}>
          <div style={{ marginBottom: 12 }}>
            ...
            <input type="text" value={patientId} onChange={e => setPatientId(e.target.value)} placeholder="P-2024-XXXXX" style={inputStyle} onFocus={focusIn} onBlur={focusOut} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={labelStyle}>Doğum tarixi</label>
            <input type="date" value={dateOfBirth} onChange={e => setDob(e.target.value)} style={inputStyle} onFocus={focusIn} onBlur={focusOut} />
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '18px 0' }}>
          ...
          <span style={{ fontSize: 12, color: '#9ca3af' }}>və ya</span>
          <div style={{ flex: 1, height: 1, background: '#e2e8f0' }} />
        </div>
        ...
        <form onSubmit={handlePhoneSearch}>
          <div style={{ marginBottom: 12 }}>
            ...
            <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="+994 XX XXX XX XX" style={inputStyle} onFocus={focusIn} onBlur={focusOut} />
          </div>
          <button type="submit" disabled={loading} style={{ width: '100%', padding: '12px', borderRadius: 12, border: `2px solid ${TEAL}`, background: 'transparent', color: TEAL, fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: FONT, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {loading && <Spinner />} SMS ilə Axtar
          </button>
        </form>
